"""
train_bert_no_preprocessing.py

Trains a BERT model on the reddit_data_annotated.csv dataset
*without any text preprocessing* steps. The model, tokenizer,
classification report, and training history are saved in
./bert_experiment_output_no_preprocessing/.
...
Usage:
  python train_bert_no_preprocessing.py
"""
...
TEXT_COLUMN = 'body'           # same text column
LABEL_COLUMN = 'is_hate_speech'
TEST_SPLIT_SIZE = 0.2
BERT_MODEL_NAME = 'bert-base-multilingual-cased'  # or any other

# Output folder for this "no preprocessing" experiment
MODEL_SAVE_PATH = './bert_experiment_output_no_preprocessing'
...
def encode_texts(tokenizer, texts, labels, max_len=128):
    ...
    without any manual text cleaning.
    """
    encodings = tokenizer(
        texts.tolist(),
        truncation=True,
...
def main():
    ...
    # We assume the CSV has columns "body" and "is_hate_speech"
    
    # -----------------------------------------------------------------------
    # 2) NO preprocessing. We simply keep the text as is.
    # -----------------------------------------------------------------------
    # So do *not* call any "preprocess_data" function
    ...
    # -----------------------------------------------------------------------
    # 4) Load a BERT tokenizer (no custom cleaning)
    # -----------------------------------------------------------------------
